...
turbine = Turbine(wind.speed_vector, TURBINE_DIAMETER, HUB_HEIGHT, CP, DT_EFFICIENCY, DOWN_TIME_PERCENTAGE)
# compute wind power
wind.wind_power_distribution(turbine.area)
# compute turbine rated power
turbine.calculate_rated_power(wind.speed_rated, RHO)
...
turbine.power_curve(wind.speed_cutout, wind.speed_rated, RHO)
...
print(f'Cut-in speed: {turbine.speed_cutin} m/s, Rated speed: {turbine.speed_rated} m/s, Cut-out speed: {turbine.speed_cutout} m/s\n')
...
nacelle_weight = NACELLE_WEIGHT_POWER * turbine.rated_power/1000
...
with open('report.txt', mode='w') as file:
    ...
    file.write('\n')
    file.write(f'\nTurbine rated power: {turbine.rated_power/1000:.2f} kW')
    file.write(f'\nCut-in speed: {turbine.speed_cutin} m/s, Rated speed: {turbine.speed_rated} m/s, Cut-out speed: {turbine.speed_cutout} m/s')
    file.write('\n')
